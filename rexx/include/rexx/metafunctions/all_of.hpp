#ifndef REXX_ALL_OF_HPP
#define REXX_ALL_OF_HPP
...
namespace rexx::metafunctions {
    ...
    struct all_of<std::tuple<>, P> {
        static constexpr bool value = false;
    };

    template<typename T, template<class> class P>
    struct all_of<std::tuple<T>, P> {
        static constexpr bool value = P<T>::value;
    };

    template<typename T, typename... Ts, template<class> class P>
    struct all_of<std::tuple<T, Ts...>, P> {
        static constexpr bool value = P<T>::value && all_of<std::tuple<Ts...>, P>::value;
    };
